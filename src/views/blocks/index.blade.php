...
<div class="ts-menu-bar border-bottom">
    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <a class="btn btn-primary" href="{{ route('app.admin.blocks.create') }}">{{ trans('pageblok::blocks.add.block') }}</a>
                <button class="btn btn-danger extra-actions" style="display: none;" type="submit">{{ trans('pageblok::blocks.delete') }}</button>
            </div>
            <div class="col-md-4">
                @if($groups)
                <div class="dropdown pull-right">
                    ...
                    <li role="menuitem"><a role="menuitem" tabindex="-1" href="{{ route('app.admin.blocks') }}">{{ trans('pageblok::blocks.blocks') }}</a></li>
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <table class="table table-striped table-hover table-condensed pageblok-item-list">
                <tr>
                    <th></th>
                    <th>{{ trans('pageblok::blocks.title') }}</th>
                    <th>{{ trans('pageblok::blocks.description') }}</th>
                    <th>{{ trans('pageblok::blocks.group') }}</th>
                    <th class="status-column">{{ trans('pageblok::blocks.status') }}</th>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        @if ($block->published)
                        <span class="label label-success">{{ trans('pageblok::blocks.published') }}</span>
                        @else
                        <span class="label label-warning">{{ trans('pageblok::blocks.draft') }}</span>
                        @endif
            ...
            @else
            <p class="alert alert-info">{{ trans('pageblok::blocks.you.have.no.blocks') }} <a href="{{ route('app.admin.blocks.create') }}" class="btn btn-primary">{{ trans('pageblok::blocks.add.block') }}</a></p>
            @endif
        </div>
    </div>
    <div class="row">
